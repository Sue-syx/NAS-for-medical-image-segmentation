    ...
    for image_name in images:
        if type == 'CT': # two types batch
            if 'IMG' in image_name:
                id = "%03d" % (int(image_name[:-4].split('-')[-1][2:])-1)
                image_mask_name = 'liver_GT_' + id + '.png'
            else:
...
class CHAOS(BaseDataset):
    ...
    def __init__(self, root,  split='train', mode=None):
      ...
      # The MR image is too challenge!
        ...
        self.random_center_crop = transforms.Resize(size=self.CROP_SIZE)
        self.to_tensor = ToTensor()
        self.img_normalize = transforms.Normalize(self.mean, self.std)
